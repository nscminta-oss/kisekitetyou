...
def generate_daily_summary_via_gemini(
    ...
    prompt = (
        ...
        "称号は15文字以内、理由は60文字以内、次への一言(nextAction)は40文字以内。"
        "前置きや説明文は付けないでください。"
    )
    ...
    try:
        ...
        raise GeminiGenerationError(f"Gemini call failed: {type(e).__name__}") from e
    ...
    try:
        ...
        raise GeminiGenerationError(f"Malformed Gemini response: {type(e).__name__}") from e
