...
async function authHeader(): Promise<Record<string, string>> {
  ...
  // getSession() はキャッシュされたセッションを返すだけで、
  // 期限切れ間近でも自動更新されないことがある。その状態のトークンを
  // バックエンドに投げると Supabase 側で 401 になり、
  // フロントが再試行するたびに 401 を量産してしまう（実際にログで確認済み）。
  // → 残り60秒を切っていたら、投げる前にこちらから更新しておく。
  const expiresAt = session?.expires_at // 秒単位のUNIX時刻
